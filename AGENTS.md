...
- **상대의 신원을 확인하고 붙는다(TOFU).** 처음 본 호스트는 신원을 기록하고 통과시키고, 그 뒤 달라지면 막는다. SSH는 호스트 키, WinRM(TLS)은 서버 인증서이며 기록은 `remote_host_keys`에 **기관별로** 둔다 — 사설망 주소는 기관마다 다른 기계다.
  - 새 원격 채널을 추가하면 `RemoteHostKeyStore.verify`를 거치게 한다. 검증을 건너뛰면 그 주소에 답하는 누구에게나 볼트에서 꺼낸 평문 자격증명을 건네는 것이 된다.
  - 신원 기록을 지우는 동작은 자원 관리자만, **감사 기록과 함께**. 중간자 공격의 마지막 단계가 기록을 지우는 것이다.
  - WinRM은 **5985(문서화된 HTTP 기본값)만 평문**이고 나머지 포트는 TLS로 간주한다. Basic 인증이라 평문이면 비밀번호가 그대로 선로에 실린다.

## 감사 로그 (PAM 필수)

- 자격증명 열람·솔루션 제어·권한 변경·로그인/로그인 실패 등 특권 행위는 **누가·언제·무엇을·결과**를 기록한다.
  - 로그인 실패는 `USER_LOGIN_FAIL`로 남는다. **행위자(`actor_user_id`)는 없을 수 있다** — 없는 계정으로의 시도가 그렇고, 계정을 훑는 공격이 정확히 그 형태라 그것이 가장 중요한 신호다(V72). 감사 기록을 읽는 화면은 행위자가 null인 줄을 견뎌야 한다.
  - 반복 실패는 `LoginThrottle`이 막는다(계정 5회 / 출발지 20회, 15분 창, **시간 경과로만 해제**). 출발지 임계값이 높은 이유는 한 사무실이 공인 IP 하나를 쓰기 때문이다. 관리자 해제 화면은 두지 않는다 — 그 화면이 로그인 뒤에 있어 마지막 관리자가 잠기면 아무도 열 수 없다.
...
- 브랜치 계층: `main`(**실제 배포용**) → `developer`(통합) → `feature/<이름>`(작업). **`main`에는 직접 커밋하지 않는다** — 배포 시점에만 `developer`를 반영한다.
...
- 원격은 `origin`(github.com/jihoonhan394/moa). 머지 후 `git push origin developer`. (옛 스터디 저장소 `mtcmhjh20/moa`는 아카이브 — 전체 히스토리는 로컬 번들 `moa-archive-20260922.bundle`에 보존.)
